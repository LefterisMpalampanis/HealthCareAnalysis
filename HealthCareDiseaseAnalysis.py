...
def display_disease_info(disease_info):
    """
    Function to display disease information in a structured way using Streamlit.
    """
    try:
        info = json.loads(disease_info)

        recovery_rate = float(info['statistics']["recovery_rate"].strip('%'))
        mortality_rate = float(info['statistics']["mortarility_rate"].strip('%'))
        ...
        for options, description in recovery_options.items():
            st.subheader(option)
        ...
        medication_count = 1
        for options, description in medication.items():
